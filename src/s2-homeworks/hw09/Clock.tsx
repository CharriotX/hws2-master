...
function Clock() {
    ...
    const start = () => {
        ...
        if (timerId === undefined) {
            clockUpdate()
            let clockInterval = setInterval(clockUpdate, 1000)
            setTimerId(Number(clockInterval))
    ...
    const stringDate = `${String(date.getDate().toLocaleString()).padStart(2, "0")}.${String(date.toLocaleString('en-US', { month: 'numeric' })).padStart(2, "0")}.${String(date.getFullYear())}` || <br /> // день.месяц.год (01.02.2022) // пишут студенты, варианты 01.02.0123/01.02.-123/01.02.12345 не рассматриваем
